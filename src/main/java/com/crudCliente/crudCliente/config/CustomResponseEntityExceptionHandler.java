...
public class CustomResponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<Object> handleResourceNotFoundException(DataIntegrityViolationException ex, WebRequest request) {
		Erro error;
		if(ex.getMessage().contains("unique_cpf")) {
			error = new Erro("cpf já existe em nossa base de dados", ExceptionUtils.getStackTrace(ex));
		}else {
			error = new Erro("cpf com tamanho maior ou menor do que 11", ExceptionUtils.getStackTrace(ex));
		}
